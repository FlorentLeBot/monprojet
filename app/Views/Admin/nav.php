<body>
    <div class="global-container">

        <nav id="side-nav">
            <div class="nav-logo">
                <!-- <img src="images/logo.svg" alt="" /> -->
                <h1>User</h1>
            </div>
            <a href="#" id="dashboard" class="bloc-link">
                <i class="fa-brands fa-dashcube"></i>
                <span class="nav-links">Dashboard</span>
            </a>
            <a href="index.php" id="root" class="bloc-link active">
                <i class="fa-solid fa-envelope"></i>
                <span class="nav-links">Messenger</span>
            </a>
            <a href="#" id="database" class="bloc-link">
                <i class="fa-solid fa-database"></i>
                <span class="nav-links">Database</span>
            </a>
            <a href="#" id="calendar" class="bloc-link">
                <i class="fa-solid fa-calendar-days"></i>
                <span class="nav-links">Calendar</span>
            </a>
        </nav>